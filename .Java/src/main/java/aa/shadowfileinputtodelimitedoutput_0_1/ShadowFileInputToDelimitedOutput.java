...
package aa.shadowfileinputtodelimitedoutput_0_1;
...
public class ShadowFileInputToDelimitedOutput implements TalendJob {
	...
	public static class RowStruct implements
		...
		final static byte[] commonByteArrayLock_AA_ShadowFileInputToDelimitedOutput = new byte[0];
		static byte[] commonByteArray_AA_ShadowFileInputToDelimitedOutput = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_AA_ShadowFileInputToDelimitedOutput.length) {
					if (length < 1024
							&& commonByteArray_AA_ShadowFileInputToDelimitedOutput.length == 0) {
						commonByteArray_AA_ShadowFileInputToDelimitedOutput = new byte[1024];
					} else {
						commonByteArray_AA_ShadowFileInputToDelimitedOutput = new byte[2 * length];
					}
				}
				dis.readFully(
						commonByteArray_AA_ShadowFileInputToDelimitedOutput, 0,
						length);
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_AA_ShadowFileInputToDelimitedOutput) {
		...
		try {
			...
			if (resumeEntryMethodName == null || resumeIt || globalResumeTicket) {// start
				...
				fileName_tFileOutputDelimited = (new java.io.File(
						"E:/NAK/Workshop/talend/AA/temp/preview/out_zp.csv"))
				...
				// 'context.fs' or 'globalMap.get("fs")'.
				if (((String) ",").length() > 0) {
					fieldSeparator_tFileInputDelimited = ((String) ",")
							.toCharArray();
				} else {
					throw new IllegalArgumentException(
							"Field Separator must be assigned a char.");
				...
				Object filename_tFileInputDelimited = /**
				 * Start field
				 * tFileInputDelimited:FILENAME
				 */
				"E:/NAK/data/out_zp.csv"/**
				 * End field tFileInputDelimited:FILENAME
				 */
				;
				...
				try {
					...
					try {// TD110 begin
						...
						if ((rowSeparator_tFileInputDelimited[0] != '\n')
							...
							csvReadertFileInputDelimited.setLineEnd(""
									+ rowSeparator_tFileInputDelimited[0]);

						csvReadertFileInputDelimited.setQuoteChar(' ');

						// ?????doesn't work for other escapeChar
						// the default escape mode is double escape
						csvReadertFileInputDelimited
								.setEscapeChar(csvReadertFileInputDelimited
										.getQuoteChar());
						...
						if (footer_tFileInputDelimited > 0) {
							...
							int lastLineTemptFileInputDelimited = totalLinetFileInputDelimited
									...
									: totalLinetFileInputDelimited
											- footer_tFileInputDelimited;
							if (lastLinetFileInputDelimited > 0) {
								lastLinetFileInputDelimited = lastLinetFileInputDelimited < lastLineTemptFileInputDelimited ? lastLinetFileInputDelimited
					...
					while (limittFileInputDelimited != 0
							&& csvReadertFileInputDelimited != null
							&& csvReadertFileInputDelimited.readNext()) {
						rowstate_tFileInputDelimited.reset();

						rowtFileInputDelimited = csvReadertFileInputDelimited
								.getValues();
						...
						if (limittFileInputDelimited > 0
								&& outputLinetFileInputDelimited > limittFileInputDelimited) {
							break;
						}
						...
						try {
							...
							if (((String) ",").length() > 0) {
								fieldSeparator_tFileInputDelimited_ListType = ((String) ",")
										.toCharArray();
							} else {
								throw new IllegalArgumentException(
										"Field Separator must be assigned a char.");
							}
							if (rowtFileInputDelimited.length == 1
									&& ("\015")
											.equals(rowtFileInputDelimited[0])) {// empty
																					// line
																					// when
																					// row
																					// separator
								...
								int columnIndexWithD_tFileInputDelimited = 0; // Column
																				// Index
		...
		try {
			...
			java.io.InputStream inContext = ShadowFileInputToDelimitedOutput.class
					.getClassLoader().getResourceAsStream(
							"aa/shadowfileinputtodelimitedoutput_0_1/contexts/"
...
/************************************************************************************************
 * 44941 characters generated by Talend Open Studio for Big Data on the June 25,
 * 2016 8:47:53 PM IST
 ************************************************************************************************/
